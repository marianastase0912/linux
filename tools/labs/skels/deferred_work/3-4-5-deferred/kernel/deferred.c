...
static void handler_work(struct work_struct *w)
{
	alloc_io();
}
...
static long deferred_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case MY_IOCTL_TIMER_SET:
			/* TODO 2: set flag */
			my_data->flag = MY_IOCTL_TIMER_SET;
			/* TODO 1: schedule timer */
			pr_info("ASTA SUNT EU ARG: %ld", arg);
			if (arg > 0)
		...
		case MY_IOCTL_TIMER_ALLOC:
			...
			my_data->flag = MY_IOCTL_TIMER_ALLOC;
			mod_timer(&my_data->timer, jiffies + HZ * arg);
			break;
...
static int deferred_init(void)
{
	...
	INIT_WORK(&dev.work, handler_work);
