				     ...
				     const struct blk_mq_queue_data *bd)
{
	...
	if (rq_data_dir(rq) == 0)
	{
		pr_info("Request received read --- start sector: [%lld] --- total size: [%d] --- data size: [%d] \n",
			blk_rq_pos(rq), blk_rq_bytes(rq), blk_rq_cur_bytes(rq));
	}
	if (rq_data_dir(rq) == 1)
	{
		pr_info("Request received write --- start sector: [%lld] --- total size: [%d] --- data size: [%d] \n", 
				blk_rq_pos(rq), blk_rq_bytes(rq), blk_rq_cur_bytes(rq));
	}
